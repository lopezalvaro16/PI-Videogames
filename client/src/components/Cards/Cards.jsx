...
const Cards = (props) => {
  ...
  const pageSelected = (event) => {
    ...
    pageNumberIndex >= 0 &&
      pageNumberIndex < pageNumber.length &&
  ...
  useEffect(() => {
    error === "Failed to fetch" ||
      (error === "Network Error" && navigate("/home"));
  }, [error]);
  ...
  useEffect(() => {
    ...
    allVideoGames.length ? dispatch(isLoadingChange(false)) : navigate("/home");
  ...
  useEffect(() => {
    ...
    currentPage / 15 + 1 === pageNumber.length
        ...
        <div>
          ...
          <div
            className={`${style.divCards} ${
              filteredGames.length < 6 && style.divCardsLenght
            }`}
          >
            ...
            {pageNumber?.map((num, index) => (
              <button
                ...
                className={`${style.buttonPage} ${
                  currentPage === index * 15 && style.currentPageButton
                }`}
              >
                {num}
              </button>
